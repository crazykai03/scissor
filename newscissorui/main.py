...
def optionpress(event):
    global pre_function_time
    pre_function_time = time.time()
    canvas1.itemconfig(option_btn, image = option)
    rgb_pop_up()
...
def changepress(event):
    global pre_function_time
    pre_function_time = time.time()
    canvas2.itemconfig(change_btn, image = change_tap)

def changerelease(event):
    global pre_function_time
    pre_function_time = time.time()
    canvas2.itemconfig(change_btn, image=change)
    command[4] = 0xC6


def confirmpress(event):
    global pre_function_time
    pre_function_time = time.time()
    canvas2.itemconfig(option_btn, image = confirm_tap)

def confirmrelease(event):
    global pre_function_time
    pre_function_time = time.time()
    canvas2.itemconfig(confirm_btn, image=confirm)
    rgb_pop_up_release()
...
def unbind_btn():
    ...
    canvas1.tag_unbind(option_btn, '<Button-1>' )
...
def bind_btn():
    ...
    canvas1.tag_bind(option_btn, '<Button-1>', optionpress)


def rgb_pop_up():
    canvas1.pack_forget()
    canvas2.pack()
    canvas2.tag_bind(change_btn, '<Button-1>', changepress)
    canvas2.tag_bind(change_btn, '<ButtonRelease-1>', changerelease)
    canvas2.tag_bind(confirm_btn, '<Button-1>', confirmpress)
    canvas2.tag_bind(confirm_btn, '<ButtonRelease-1>', confirmrelease)

def rgb_pop_up_release():
    canvas2.pack_forget()
    canvas1.pack()
    canvas2.tag_unbind(change_btn, '<Button-1>' )
    canvas2.tag_unbind(change_btn, '<ButtonRelease-1>' )
    canvas2.tag_unbind(confirm_btn, '<Button-1>' )
    canvas2.tag_unbind(confirm_btn, '<ButtonRelease-1>' )
...
light1= PhotoImage(file="light1.png")
light2= PhotoImage(file="light2.png")
light3= PhotoImage(file="light3.png")
rgb1= PhotoImage(file="rgb.png")
all_light = PhotoImage(file="alllight.png")
...
up_tap= PhotoImage(file="up_tap.png")
down_tap= PhotoImage(file="down_tap.png")
stop_tap = PhotoImage(file="stop_tap.png")
light1_tap= PhotoImage(file="light1_tap.png")
light2_tap= PhotoImage(file="light2_tap.png")
light3_tap= PhotoImage(file="light3_tap.png")
rgb1_tap= PhotoImage(file="rgb_tap.png")
all_light_tap = PhotoImage(file="all_tap.png")
lock_tap= PhotoImage(file="lock_tap.png")
unlock_tap= PhotoImage(file = "unlock_tap.png")
...
up_btn = canvas1.create_image(16+offset_x,16+offset_y,image=up)
down_btn = canvas1.create_image(16+offset_x,248+offset_y,image=down)
stop_btn =  canvas1.create_image(640+offset_x,16+offset_y,image=stop)
lock_btn =  canvas1.create_image(640+offset_x,248+offset_y,image=lock)

light1_btn =  canvas1.create_image(176+offset_y,16+offset_y,image=light1)
light2_btn =  canvas1.create_image(176+offset_y,248+offset_y,image=light2)
light3_btn =  canvas1.create_image(408+offset_y,16+offset_y,image=light3)
rgb1_btn =  canvas1.create_image(408+offset_y,248+offset_y,image=rgb1)
all_light_btn =  canvas1.create_image(280+120,120+120,image=all_light)
